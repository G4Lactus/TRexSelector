...
inline void bind_utils_module(py::module& m) {
    ...
    m.def("true_positives_count",
          &trex::utils::eval::counts::true_positives_count,
          py::arg("selected_indices"), py::arg("true_support"),
          "Count true positives (selected AND in true support)");

    m.def("false_positives_count",
          &trex::utils::eval::counts::false_positives_count,
          py::arg("selected_indices"), py::arg("true_support"),
          "Count false positives (selected but not in true support)");

    m.def("false_negatives_count",
          &trex::utils::eval::counts::false_negatives_count,
          py::arg("selected_indices"), py::arg("true_support"),
          "Count false negatives (in true support but not selected)");

    m.def("f1_score",
          &trex::utils::eval::composites::f1_score,
          py::arg("precision"), py::arg("recall"),
