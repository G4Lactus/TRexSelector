...
TRexControlParameter parse_control_parameter(const Rcpp::List& control) {
    ...
    if (control.containsElementNamed("lloop_strategy")) {
        ...
        } else if (strategy == "PERMUTATION_SEEDED") {
            params.lloop_strategy = LLoopStrategy::PERMUTATION_SEEDED;
        } else if (strategy == "SEEDED") {
            params.lloop_strategy = LLoopStrategy::SEEDED;
        } else {
...
XPtr<RTRexSelector> trex_selector_create(
    ...
    return XPtr<RTRexSelector>(new RTRexSelector(X_map, y_map, tFDR,
