...
TRexDAControlParameter parse_da_parameter(const Rcpp::List& control) {
    ...
    // Using hc_grid_length rather than BT_grid_size
    if (control.containsElementNamed("hc_grid_length")) params.hc_grid_length = control["hc_grid_length"];
...
#else
//' @title Create TRexDASelector for CRAN
//' @noRd
// [[Rcpp::export]]
XPtr<RTRexDASelector> trex_da_create(
    Eigen::Map<Eigen::MatrixXd> X,
...
IntegerVector trex_da_get_selected_indices(XPtr<RTRexDASelector> r_ptr) {
    ...
    for(size_t i = 0; i < indices.size(); ++i) {
        res[i] = static_cast<int>(indices[i] + 1);
    }
    return res;
}
"""
...
if "trex_da_create" not in content:
    with open("src/rcpp_trex_selector.cpp", "a") as f:
        f.write(da_bindings)
