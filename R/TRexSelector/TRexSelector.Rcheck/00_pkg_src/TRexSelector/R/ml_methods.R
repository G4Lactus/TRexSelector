...
ZScoreScaler <- R6::R6Class("ZScoreScaler",
  ...
  public = list(
    ...
    get_dropped_indices = function() {
      zscore_scaler_get_dropped_indices(private$cpp_ptr)
    },
...
LpNormScaler <- R6::R6Class("LpNormScaler",
  ...
  public = list(
    ...
    initialize = function(norm_type = 2, with_mean = TRUE) {
      if (!(norm_type %in% c(1, 2))) stop("norm_type must be 1 (L1) or 2 (L2)")
    ...
    #' @return Value.
    get_dropped_indices = function() {
      lpnorm_scaler_get_dropped_indices(private$cpp_ptr)
    },
...
RidgeGCV <- R6::R6Class("RidgeGCV",
  ...
  public = list(
    ...
    solve_path = function(lambdas) {
      ridge_gcv_solve_path(private$cpp_ptr, as.numeric(lambdas))
    },
