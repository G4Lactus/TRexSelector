...
void demo_TOMP_early_stopping(bool high_dim, std::size_t T_stop = 5) {

    std::cout << "=== Demo 1: Basic T-OMP with Early Stopping ===\n";
...
void demo_TOMP_with_external_normalizer(bool high_dim, std::size_t T_stop = 5) {

    std::cout << "=== Demo 2: T-OMP with External Normalization ===\n";
...
void demo_TOMP_serialization() {
    ...
    std::cout << "=== Demo 3: T-OMP Serialization & Path Consistency ===\n\n";
...
void demo_TOMP_controlled_comparison() {
    ...
    // ============================================================
    // Step 5: Run T-OMP for both versions
    // ============================================================
    std::cout << "\n=== Step 5: Running T-OMP on in-memory data ===\n";
    TOMP_Solver tomp_inmem(X_aug_inmem_map, y_inmem_map, num_dummies, false, false, true);
    tomp_inmem.executeStep(T_stop, true);
    std::cout << "  ✓ In-memory T-OMP complete\n\n";

    std::cout << "Running T-OMP on memory-mapped data...\n";
    TOMP_Solver tomp_mmap(X_aug_mmap_map, y_mmap_map, num_dummies, false, false, true);
    tomp_mmap.executeStep(T_stop, true);
    std::cout << "  ✓ Memory-mapped T-OMP complete\n\n";
    ...
    auto path_inmem = tomp_inmem.getActions();
    auto path_mmap = tomp_mmap.getActions();

    std::cout << "In-memory path:\n  ";
    utils_talgos::print_selection(tomp_inmem, true_support);

    std::cout << "\nMemory-mapped path:\n  ";
    utils_talgos::print_selection(tomp_mmap, true_support);
...
// ============================================================================
// Demo 5: Production T-OMP Workflow
// ============================================================================

void demo_production_tomp_workflow() {

    utils_eval::print_section_header("=== Demo 5: Production T-OMP Workflow ===\n\n");
    ...
    std::cout << "T-OMP completed in " << t1.time_ms << " ms\n\n";
...
int main() {
    ...
    try {
        ...
        // T-OMP with early stopping - low and high dimensional
        const bool run_early_stopping_demo = false;
        if (run_early_stopping_demo) {
            demo_TOMP_early_stopping(/*high_dim=*/false, /*T_stop=*/10);
            demo_TOMP_early_stopping(/*high_dim=*/true, /*T_stop=*/10);
        }
        ...
        // T-OMP serialization and warm-start
        const bool run_serialization_demo = false;
        if (run_serialization_demo) demo_TOMP_serialization();


        // T-OMP controlled comparison
        const bool run_controlled_comparison = false;
        if (run_controlled_comparison) demo_TOMP_controlled_comparison();


        // T-OMP production workflow
        const bool run_production_tomp_with_mmap = true;
        if (run_production_tomp_with_mmap) demo_production_tomp_workflow();


        std::cout << "\n✓ All T-OMP demos completed\n\n";
