...
void demo_TOMP_early_stopping(bool high_dim, std::size_t T_stop = 5) {
    ...
    std::cout << "T-OMP completed full path in " << t1.time_ms << " ms ("
              << tomp.getNumSteps() << " steps)\n";

    print_selection(tomp, true_support);
    print_quality(tomp, true_support);
    std::cout << "\n\n";
}
...
void demo_TOMP_serialization() {
    std::cout << "Demo 2: T-OMP Serialization & Path Consistency Demo" << "\n";
...
int main() {
    ...
    try {
        // T-OMP with early stopping
        demo_TOMP_early_stopping(/*high_dim=*/true, /*T_stop=*/5);
        demo_TOMP_early_stopping(/*high_dim=*/false, /*T_stop=*/5);
