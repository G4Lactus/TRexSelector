...
void demo_TLASSO_early_stopping(bool high_dim, std::size_t T_stop = 5) {
    ...
    print_selection(tlasso, true_support);
    print_quality(tlasso, true_support);
...
void demo_TLASSO_serialization() {
    std::cout << "Demo 2: T-LASSO Serialization & Path Consistency Demo" << "\n";
    ...
    TLASSO_Solver solver2(X_aug_map, y_map, num_dummies, true, true, true);
    {
...
int main() {
    ...
    try {
        ...
        demo_TLASSO_early_stopping(/*high_dim=*/false, /*T_stop=*/5);
        demo_TLASSO_early_stopping(/*high_dim=*/true, /*T_stop=*/5);
