...
void demo_TSTEPWISE_early_stopping(bool high_dim, std::size_t T_stop = 5) {
    ...
    // Normalization is handled by the TSTEPWISE solver internally
...
void demo_TSTEPWISE_with_external_normalizer(bool high_dim, std::size_t T_stop = 5) {

    std::cout << "=== Demo 2: T-Stepwise with External Normalization ===\n";
    ...
    TSTEPWISE_Solver tstepwise(X_aug_map, y_map, num_dummies, false, false, true);
    auto t1 = utils_perf::profileit([&]() { tstepwise.executeStep(T_stop, /*early_stop=*/true); });
    std::cout << "T-Stepwise early stopping at T=" << T_stop << " took " << t1.time_ms << " ms\n";
...
void demo_TSTEPWISE_serialization() {
    ...
    std::cout << "=== Demo 3: T-Stepwise Serialization & Path Consistency ===\n\n";
...
void demo_TSTEPWISE_controlled_comparison() {
    ...
    const std::string X_aug_file = "tstepwise_test_X_aug.bin";
    const std::string y_file = "tstepwise_test_y.bin";
    ...
    // ============================================================
    // Step 5: Running T-Stepwise for both versions
    // ============================================================
    std::cout << "\n=== Step 5: Running T-Stepwise on in-memory data ===\n";
    TSTEPWISE_Solver tstepwise_inmem(X_aug_inmem_map, y_inmem_map, num_dummies, false, false, true);
    tstepwise_inmem.executeStep(T_stop, true);
    std::cout << "  ✓ In-memory T-Stepwise complete\n\n";

    std::cout << "Running T-Stepwise on memory-mapped data...\n";
    TSTEPWISE_Solver tstepwise_mmap(X_aug_mmap_map, y_mmap_map, num_dummies, false, false, true);
    tstepwise_mmap.executeStep(T_stop, true);
    std::cout << "  ✓ Memory-mapped T-Stepwise complete\n\n";
    ...
    auto path_inmem = tstepwise_inmem.getActions();
    auto path_mmap = tstepwise_mmap.getActions();

    std::cout << "In-memory path:\n  ";
    utils_talgos::print_selection(tstepwise_inmem, true_support);

    std::cout << "\nMemory-mapped path:\n  ";
    utils_talgos::print_selection(tstepwise_mmap, true_support);
...
// ============================================================================
// Demo 5: Production T-Stepwise Workflow
// ============================================================================

void demo_production_tstepwise_workflow() {

    utils_eval::print_section_header("=== Demo 5: Production T-Stepwise Workflow ===\n\n");
    ...
    std::cout << "T-Stepwise completed in " << t1.time_ms << " ms\n\n";
...
int main() {
    ...
    try {
        ...
        // T-Stepwise with early stopping - low and high dimensional
        const bool run_early_stopping_demo = false;
        if (run_early_stopping_demo) {
            demo_TSTEPWISE_early_stopping(/*high_dim=*/false, /*T_stop=*/10);
            demo_TSTEPWISE_early_stopping(/*high_dim=*/true, /*T_stop=*/10);
        }
        ...
        // T-Stepwise serialization and warm-start
        const bool run_serialization_demo = false;
        if (run_serialization_demo) demo_TSTEPWISE_serialization();


        // T-Stepwise controlled comparison
        const bool run_controlled_comparison = false;
        if (run_controlled_comparison) demo_TSTEPWISE_controlled_comparison();


        // T-Stepwise production workflow
        const bool run_production_tstepwise_with_mmap = true;
        if (run_production_tstepwise_with_mmap) demo_production_tstepwise_workflow();
