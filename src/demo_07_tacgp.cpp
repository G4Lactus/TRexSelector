...
void demo_TACGP_early_stopping(bool high_dim, std::size_t T_stop = 5) {
...
void demo_TACGP_with_external_normalizer(bool high_dim, std::size_t T_stop = 5) {

    std::cout << "=== Demo 2: T-ACGP with External Normalization ===\n";
...
void demo_TACGP_serialization() {
    ...
    std::cout << "=== Demo 3: T-ACGP Serialization & Path Consistency ===\n\n";
    ...
    TACGP_Solver solver_ref(X_aug_map, y_map, num_dummies, true, true, true);
    ...
    {
        TACGP_Solver solver1(X_aug_map, y_map, num_dummies, true, true, true);
    ...
    {
        TACGP_Solver solver2 = TACGP_Solver::load(filename.c_str(), X_aug_map);
...
void demo_TACGP_controlled_comparison() {
    ...
    // ============================================================
    // Step 5: Run T-ACGP for both versions
    // ============================================================
    std::cout << "\n=== Step 5: Running T-ACGP on in-memory data ===\n";
    TACGP_Solver tacgp_inmem(X_aug_inmem_map, y_inmem_map, num_dummies, false, false, true);
    tacgp_inmem.executeStep(T_stop, true);
    std::cout << "  ✓ In-memory T-ACGP complete\n\n";

    std::cout << "Running T-ACGP on memory-mapped data...\n";
    TACGP_Solver tacgp_mmap(X_aug_mmap_map, y_mmap_map, num_dummies, false, false, true);
    tacgp_mmap.executeStep(T_stop, true);
    std::cout << "  ✓ Memory-mapped T-ACGP complete\n\n";
    ...
    auto path_inmem = tacgp_inmem.getActions();
    auto path_mmap = tacgp_mmap.getActions();

    std::cout << "In-memory path:\n  ";
    utils_talgos::print_selection(tacgp_inmem, true_support);

    std::cout << "\nMemory-mapped path:\n  ";
    utils_talgos::print_selection(tacgp_mmap, true_support);
...
// ============================================================================
// Demo 5: Production T-ACGP Workflow
// ============================================================================

void demo_production_tacgp_workflow() {

    utils_eval::print_section_header("=== Demo 5: Production T-ACGP Workflow ===\n\n");
    ...
    std::cout << "T-ACGP completed in " << t1.time_ms << " ms\n\n";
...
int main() {
    ...
    try {
        ...
        // T-ACGP with early stopping - low and high dimensional
        const bool run_early_stopping_demo = false;
        if (run_early_stopping_demo) {
            demo_TACGP_early_stopping(/*high_dim=*/false, /*T_stop=*/10);
            demo_TACGP_early_stopping(/*high_dim=*/true, /*T_stop=*/10);
        }
        ...
        // T-ACGP serialization and warm-start
        const bool run_serialization_demo = false;
        if (run_serialization_demo) demo_TACGP_serialization();


        // T-ACGP controlled comparison
        const bool run_controlled_comparison = false;
        if (run_controlled_comparison) demo_TACGP_controlled_comparison();


        // T-ACGP production workflow
        const bool run_production_tacgp_with_mmap = true;
        if (run_production_tacgp_with_mmap) demo_production_tacgp_workflow();


        std::cout << "\n✓ All T-ACGP demos completed\n\n";
