...
void demo_TENET_early_stopping(bool high_dim, std::size_t T_stop = 5) {

    std::cout << "=== Demo 1: Basic T-Elastic Net with Early Stopping ===\n";
    ...
    // Normalization is handled by the TENET solver internally
    ...
    std::cout << "T-Elastic Net early stopping at T=" << T_stop << " took " << t1.time_ms << " ms\n";
...
void demo_TENET_with_external_normalizer(bool high_dim, std::size_t T_stop = 5) {

    std::cout << "=== Demo 2: T-Elastic Net with External Normalization ===\n";
    ...
    TENET_Solver tenet(X_aug_map, y_map, num_dummies, lambda2, false, false, true);
    auto t1 = utils_perf::profileit([&]() { tenet.executeStep(T_stop, /*early_stop=*/true); });
    std::cout << "T-Elastic Net early stopping at T=" << T_stop << " took " << t1.time_ms << " ms\n";
...
void demo_TENET_serialization() {
    ...
    std::cout << "=== Demo 3: T-Elastic Net Serialization & Path Consistency ===\n\n";
...
void demo_TENET_controlled_comparison() {
    ...
    const std::string X_aug_file = "tenet_test_X_aug.bin";
    const std::string y_file = "tenet_test_y.bin";
    ...
    // ============================================================
    // Step 5: Run T-Elastic Net for both versions
    // ============================================================
    std::cout << "\n=== Step 5: Running T-Elastic Net on in-memory data ===\n";
    TENET_Solver tenet_inmem(X_aug_inmem_map, y_inmem_map, num_dummies, lambda2, false, false, true);
    tenet_inmem.executeStep(T_stop, true);
    std::cout << "  ✓ In-memory T-Elastic Net complete\n\n";

    std::cout << "Running T-Elastic Net on memory-mapped data...\n";
    TENET_Solver tenet_mmap(X_aug_mmap_map, y_mmap_map, num_dummies, lambda2, false, false, true);
    tenet_mmap.executeStep(T_stop, true);
    std::cout << "  ✓ Memory-mapped T-Elastic Net complete\n\n";
    ...
    auto path_inmem = tenet_inmem.getActions();
    auto path_mmap = tenet_mmap.getActions();

    std::cout << "In-memory path:\n  ";
    utils_talgos::print_selection(tenet_inmem, true_support);

    std::cout << "\nMemory-mapped path:\n  ";
    utils_talgos::print_selection(tenet_mmap, true_support);
...
// ============================================================================
// Demo 5: Production T-Elastic Net Workflow
// ============================================================================

void demo_production_tenet_workflow() {
    ...
    const std::string X_file = "tenet_production_X.bin";
    const std::string y_file = "tenet_production_y.bin";
    const std::string X_aug_file = "tenet_production_X_aug.bin";
    ...
    // ========================================================================
    // Step 5: Run T-Elastic Net
    // ========================================================================
    std::cout << "\n=== Step 5: Running T-Elastic Net ===\n";

    TENET_Solver tenet(X_aug, y, num_dummies, lambda2, false, false, true);

    auto t1 = utils_perf::profileit([&]() {
        tenet.executeStep(T_stop, true);
    });

    std::cout << "T-Elastic Net completed in " << t1.time_ms << " ms\n\n";
...
int main() {
    ...
    try {
        ...
        // T-Elastic Net with early stopping - low and high dimensional
        const bool run_early_stopping_demo = false;
        if (run_early_stopping_demo) {
            demo_TENET_early_stopping(/*high_dim=*/false, /*T_stop=*/10);
            demo_TENET_early_stopping(/*high_dim=*/true, /*T_stop=*/10);
        }


        // T-Elastic Net with external normalization
        const bool run_external_normalizer_demo = false;
        if (run_external_normalizer_demo) {
            demo_TENET_with_external_normalizer(/*high_dim=*/false, /*T_stop=*/5);
            demo_TENET_with_external_normalizer(/*high_dim=*/true, /*T_stop=*/5);
        }


        // T-Elastic Net serialization and warm-start
        const bool run_serialization_demo = false;
        if (run_serialization_demo) demo_TENET_serialization();


        // T-Elastic Net controlled comparison
        const bool run_controlled_comparison = false;
        if (run_controlled_comparison) demo_TENET_controlled_comparison();


        // T-Elastic Net production workflow
        const bool run_production_tenet_with_mmap = true;
        if (run_production_tenet_with_mmap) demo_production_tenet_workflow();
