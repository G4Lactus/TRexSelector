...
/**
 * @brief Check if the directory has enough available disk space (in bytes).
 *
 * This function is implemented for POSIX systems. On Windows, it always returns true.
 *
 * @param needed_bytes The required disk space in bytes.
 * @param dir The directory to check (default: current directory).
 * @return True if sufficient space is available or if the check is indeterminate; false otherwise.
 */
inline bool check_disk_space(std::size_t needed_bytes, const char* dir = ".") {
#if !defined(_WIN32) && !defined(_WIN64)
    struct statvfs info;
    if (statvfs(dir, &info) != 0) {
        get_default_stream() << "Warning: Could not check disk space\n";
        return true; // Return true on error to avoid false negatives
    }
    std::size_t avail = info.f_frsize * info.f_bavail;
    return avail > needed_bytes;
#else
    // Not implemented for Windows in this header; always succeeds.
    return true;
#endif
}
