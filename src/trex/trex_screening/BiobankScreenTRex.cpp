...
BiobankScreenTRex::BiobankScreenTRex(
    ...
    y_(y),
    Y_(nullptr),
    biosctrex_ctrl_(std::move(biosctrex_ctrl)),
    seed_(seed),
    verbose_(verbose)
{
...
std::vector<BiobankScreenTRexResult> BiobankScreenTRex::screenPhenotypes() {
    ...
    if (verbose_) {
        std::cout << "\n=== Biobank Screen-TRex (Algorithm 1) ===\n";
        std::cout << "Phenotypes: " << num_phenotypes << "\n";
        std::cout << "FDR range: [" << biosctrex_ctrl_.lower_bound_FDR
                  << ", " << biosctrex_ctrl_.upper_bound_FDR << "]\n";
        std::cout << "Fallback target FDR: " << biosctrex_ctrl_.target_FDR_trex << "\n\n";
    }
    ...
    for (std::size_t pheno_idx = 0; pheno_idx < num_phenotypes; ++pheno_idx) {
        ...
        Eigen::VectorXd y_col = Y_->col(pheno_idx);
        results.push_back(screenSinglePhenotype(y_col, pheno_idx));

        if (verbose_) {
            const auto& res = results.back();
            std::cout << " Method: " << res.method_used << "\n";
            std::cout << " Selected: " << res.selected_indices.size() << "\n";
            std::cout << " Est. FDR: " << std::fixed << std::setprecision(4)
                      << res.estimated_FDR << "\n";
            std::cout << "  α̂ (ordinary): " << res.estimated_FDR_screen_ordinary << "\n";
            std::cout << "  α̂_C (bootstrap): " << res.estimated_FDR_screen_bootstrap << "\n\n";
        }
    }
...
BiobankScreenTRexResult BiobankScreenTRex::screenSinglePhenotype(
    ...
    ScreenTRexSelector sctrex_bootstrap(
        *X_,
        y_map,
        biosctrex_ctrl_.screen_ctrl,
        biosctrex_ctrl_.trex_ctrl,
        biosctrex_ctrl_.solver_ctrl,
        seed_,
        false
    );
    ...
    ScreenTRexSelector sctrex_ordinary(
        *X_,
        y_map,
        biosctrex_ctrl_.screen_ctrl,
        biosctrex_ctrl_.trex_ctrl,
...
BiobankScreenTRexResult BiobankScreenTRex::applyDecisionLogic(
    const ScreenTRexSelectionResult& sctrex_result_bootstrap,
    const ScreenTRexSelectionResult& sctrex_result_ordinary,
    const Eigen::VectorXd& y,
    std::size_t phenotype_index
) {
    ...
    // ---------------------------------------------------
    if (verbose_) {
        std::cout << "  [Info] Screen-TRex estimates (Ord: " << alpha_hat
...
BiobankScreenTRex::BiobankScreenTRexStatistics BiobankScreenTRex::getBiobankScreenTRexResult(
    ...
    for (const auto& res : results) {
        ...
        sum_selected += res.selected_indices.size();
    ...
    stats.avg_estimated_FDR = sum_fdr / results.size();
    stats.avg_selected_count = sum_selected / results.size();
