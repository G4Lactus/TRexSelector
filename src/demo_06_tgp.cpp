...
void demo_TGP_early_stopping(bool high_dim, std::size_t T_stop = 5) {
    ...
    std::cout << "T-GP completed full path in " << t1.time_ms << " ms ("
              << tgp.getNumSteps() << " steps)\n";

    print_selection(tgp, true_support);
    print_quality(tgp, true_support);
    std::cout << "\n\n";
}
...
void demo_TGP_serialization() {
    std::cout << "Demo 2: T-GP Serialization & Path Consistency Demo" << "\n";
