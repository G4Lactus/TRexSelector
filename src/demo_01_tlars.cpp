...
void demo_TLARS_early_stopping(bool high_dim, std::size_t T_stop = 5) {
    ...
    print_selection(tlars, true_support);
    print_quality(tlars, true_support);
    std::cout << "\n\n";
}
...
void demo_TLARS_serialization() {
    std::cout << "Demo 2: T-LARS Serialization & Path Consistency Demo" << "\n";
...
int main() {
    ...
    try {
        ...
        demo_TLARS_early_stopping(/*high_dim=*/false, /*T_stop=*/5);
        demo_TLARS_early_stopping(/*high_dim=*/true, /*T_stop=*/5);
