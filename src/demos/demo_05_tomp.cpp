...
// ============================================================================
/**
 * @file demo_05_tomp.cpp
 *
 * @brief Demonstration of T-OMP (Terminating Orthogonal Matching Pursuit) solver.
 *
...
// ============================================================================
// Demo 1: Basic T-OMP with Early Stopping
// ============================================================================

void demo_TOMP_early_stopping(bool high_dim, bool rnd_coef, std::size_t T_stop) {

    // Print Demo Header
    cdiagnost::print_section_header("Demo 1: Basic T-OMP with Early Stopping");

    // Setup
    const std::size_t n = high_dim ? 1000 : 5000;;
    const std::size_t p = high_dim ? 5000 : 1000;
    // Note: 10, 100, 1000 scale file
    ...
    // Run T-OMP solver with internal normalization
    TOMP_Solver tomp(X_aug_map, y_map, num_dummies,
                      /*normalization=*/true,
                      /*intercept=*/true,
                      /*verbose=*/true
                    );
    ...
    tomp.executeStep(T_stop, /*early_stop=*/true);

    // Results
    // Print selection path
    cdiagnost::print_selection(tomp, true_support);
    // Print selection quality measures
    cdiagnost::print_selection_quality(tomp, true_support);
...
void demo_TOMP_with_external_normalizer(bool high_dim, bool rnd_coef, std::size_t T_stop) {

    // Print Demo Header
    cdiagnost::print_section_header("=== Demo 2: T-OMP with External Normalization ===");
    ...
    // Augment with dummies
    Eigen::MatrixXd X_aug = datagen::append_dummies_to_matrix(data.getX(), num_dummies, /*seed=*/1234);
    Eigen::VectorXd y = data.getY();
    ...
    ml::Normalizer normalizer(ml::Normalizer::NormType::L2, true);
...
void demo_TOMP_serialization() {

    // Print Demo Header
    cdiagnost::print_section_header("=== Demo 3: T-OMP Serialization & Path Consistency ===");
    ...
    cdiagnost::print_talgo_demo_config(n, p, num_dummies, T_stop_final, true_support,
                                       true_coefs, /*snr=*/snr);
    ...
    TOMP_Solver solver_ref(X_aug_map, y_map, num_dummies, true, true, true);
    solver_ref.executeStep(T_stop_final, /*early_stop=*/true);
    std::cout << "✓ Reference completed with " << solver_ref.getNumSteps() << " steps\n\n";

    std::string filename = "tomp_checkpoint.bin";

    // --------- STEP 1: Run partial path and save checkpoint (scoped) ---------
    {
        TOMP_Solver solver1(X_aug_map, y_map, num_dummies, true, true, true);
    ...
    {
        TOMP_Solver solver2 = TOMP_Solver::load(filename.c_str(), X_aug_map);
...
void demo_04_memory_mapped(bool high_dim, bool rnd_coef, std::size_t T_stop) {

    // Print Demo Header
    cdiagnost::print_section_header("=== Demo 4: T-OMP with Memory-Mapped Data ===");
    ...
    // generate augmented memory-mapped data
    std::cout << "Generating augmented memory-mapped data...\n";
    const std::string X_aug_file = "demo_tomp_X_aug.bin";
    const std::string y_file = "demo_tomp_y.bin";

    // Generate memory-mapped data
    std::cout << "Generating memory-mapped data...\n";
    datagen::SyntheticDataMappedWithDummies data(
        X_aug_file, y_file, n, p, num_dummies,
        true_support, true_coefs, snr, /*seed=*/42
    );
    std::cout << "✓ Data generated on disk\n\n";

    // Create Maps
    auto X_aug_map = data.getXAug();;
    auto y_matrix_map = data.getY();
    Eigen::Map<Eigen::VectorXd> y_map(y_matrix_map.data(), n);

    // Run T-OMP
    std::cout << "Running T-OMP on memory-mapped data...\n";
    TOMP_Solver tomp(X_aug_map, y_map, num_dummies,
                      /*normalization=*/true,
                      /*intercept=*/true,
                      /*verbose=*/true);
...
int main() {
    ...
    std::cout << "\n";
    // Print header
    cdiagnost::print_section_header("T-OMP Demo Suite");
    ...
    try {
        ...
        demo_TOMP_early_stopping(/*high_dim=*/true, /*rnd_coef=*/false, /*T_stop=*/10);
        demo_TOMP_early_stopping(/*high_dim=*/false, /*rnd_coef=*/false, /*T_stop=*/10);

        // Demo 2: External normalization
        demo_TOMP_with_external_normalizer(/*high_dim=*/false, /*rnd_coef=*/false, /*T_stop=*/10);
        demo_TOMP_with_external_normalizer(/*high_dim=*/true, /*rnd_coef=*/false, /*T_stop=*/10);

        // Demo 3: Serialization
        demo_TOMP_serialization();
