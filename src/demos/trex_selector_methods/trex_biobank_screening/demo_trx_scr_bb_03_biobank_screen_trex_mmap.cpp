// ==============================================================================
// demo_trx_05_biobank_screen_trex_mmap.cpp
// ==============================================================================
/**
 * @file demo_trx_05_biobank_screen_trex_mmap.cpp
 *
 * @brief Biobank Screen-TRex Selector (Algorithm 1) — memory-mapped variant.
 *
 * @details
 *  Repeats demo_trx_04_biobank_screen_trex_inmem.cpp with
 *  TRexControlParameter::use_memory_mapping = true.
 *
 *  Algorithm 1 internally spawns two ScreenTRexSelector instances (ordinary
 *  and bootstrap-CI) and optionally a fallback TRexSelector.  All three use
 *  memory-mapped D-file storage when use_memory_mapping is set on the shared
 *  TRexControlParameter.
 *
 *  Demo 01 — Single phenotype, memory-mapped D files.
 *  Demo 02 — Multiple phenotype screening, memory-mapped D files.
 *  Demo 03 — Monte Carlo SNR sweep, memory-mapped D files.
 */
// ==============================================================================
...
// ==============================================================================
// Shared helper: build a default BiobankScreenTRexControl (memory-mapped)
// ==============================================================================
    ...
    int K = 20, double R_boot = 1000, double ci_grid_step = 0.001)
{
    ...
    ctrl.trex_ctrl.use_memory_mapping   = true;     // <-- memory-mapped D storage
...
void demo_BiobankScreenTrex_MMap_SinglePhenotype(bool rnd_coef = false)
{
    cdiag::print_section_header(
        "Demo: Biobank Screen-TRex — Single Phenotype (Memory-Mapped)");
    ...
    std::cout << "Running Algorithm 1 (use_memory_mapping=true)...\n";
...
// ==============================================================================
// Demo 02: Multiple phenotype screening — memory-mapped
// ==============================================================================

void demo_BiobankScreenTrex_MMap_MultiplePhenotypes(bool rnd_coef = false)
{
    cdiag::print_section_header(
        "Demo: Biobank Screen-TRex — Multiple Phenotypes (Memory-Mapped)");
    ...
    std::cout << "Running Algorithm 1 for all phenotypes (use_memory_mapping=true)...\n\n";
    tbs::BiobankScreenTRex biosctrex(X_map, Y_map, ctrl, /*seed=*/42, /*verbose=*/true);
...
void demo_BiobankScreenTrex_MMap_MonteCarlo(std::size_t num_MC, bool rnd_coef)
{
    cdiag::print_section_header(
        "Demo: Biobank Screen-TRex — Monte Carlo (Memory-Mapped)");
    ...
    std::cout << "\n" << std::string(90, '=') << "\n";
    std::cout << "=== Biobank Screen-TRex Results (avg over " << num_MC
              << " runs, mmap) ===\n";
    std::cout << std::string(90, '=') << "\n\n";
...
int main()
{
    ...
    // Demo 01: single phenotype — memory-mapped
    demo_BiobankScreenTrex_MMap_SinglePhenotype(/*rnd_coef=*/false);

    // Demo 02: multiple phenotypes — memory-mapped
    // demo_BiobankScreenTrex_MMap_MultiplePhenotypes(/*rnd_coef=*/false);

    // Demo 03: Monte Carlo SNR sweep — memory-mapped
    // demo_BiobankScreenTrex_MMap_MonteCarlo(/*num_MC=*/200, /*rnd_coef=*/true);
