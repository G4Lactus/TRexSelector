// ==============================================================================
// demo_trx_03_biobank_screen_trex.cpp
// ==============================================================================
/**
 * @file demo_trx_03_biobank_screen_trex.cpp
 *
...
// ==============================================================================
// Shared helper: build a default BiobankScreenTRexControl
// ==============================================================================
    ...
    int K = 20, double R_boot = 1000, double ci_grid_step = 0.001)
{
    ...
    ctrl.trex_ctrl.use_memory_mapping   = false;
...
{
    cdiag::print_section_header("Demo: Biobank Screen-TRex — Single Phenotype");
...
{
    cdiag::print_section_header("Demo: Biobank Screen-TRex — Multiple Phenotypes");

    const std::size_t n = 300;
    const std::size_t p = 1000;
    const std::size_t q = 5;

    // Shared design matrix (Gaussian IID)
    ...
    // Build Y: each column is a separate phenotype with its own support and SNR
...
{
    cdiag::print_section_header("Demo: Biobank Screen-TRex — Monte Carlo");
    ...
    for (std::size_t snr_idx = 0; snr_idx < snr_values.size(); ++snr_idx) {
        ...
        for (std::size_t mc = 0; mc < num_MC; ++mc) {
            ...
            fdp_res[result.method_used](si)  += fdp;
            tpp_res[result.method_used](si)  += tpp;
    ...
    for (const auto& nm : method_names) {
        auto print_row = [&](const std::string& label, const Eigen::VectorXd& v, int dp = 4) {
            std::cout << std::setw(nw) << "" << std::setw(mw) << label;
            for (Eigen::Index i = 0; i < ns; ++i)
                std::cout << std::fixed << std::setprecision(dp) << std::setw(cw) << v(i);
            std::cout << "\n";
        };
