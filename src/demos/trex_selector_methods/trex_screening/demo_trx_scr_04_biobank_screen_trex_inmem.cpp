// ==============================================================================
// demo_trx_scr_04_biobank_screen_trex_inmem.cpp
// ==============================================================================
/**
 * @file demo_trx_scr_04_biobank_screen_trex_inmem.cpp
 *
 * @brief Biobank Screen-TRex Selector (Algorithm 1) — in-memory D-file variant.
 *
 * @details Dummy matrices are stored in memory (use_memory_mapping = false).
 *          See demo_trx_scr_05_biobank_screen_trex_mmap.cpp for the
 *          memory-mapped workflow.
 *
 *  Demo 01 — Single phenotype: basic Algorithm 1 functionality.
...
#include "demo_trx_scr_bb_common.hpp"
...
void demo_Biobank_SinglePhenotype()
{
    cdiag::print_section_header(
        "Demo 01: Biobank Screen-TRex — Single Phenotype (In-Memory)");
    ...
    auto ctrl = makeBiobankControl(/*use_mmap=*/false);
...
void demo_Biobank_MultiplePhenotypes()
{
    cdiag::print_section_header(
        "Demo 02: Biobank Screen-TRex — Multiple Phenotypes (In-Memory)");
...
void demo_Biobank_MonteCarlo(std::size_t num_MC)
{
    std::cout.setf(std::ios::unitbuf);
    cdiag::print_section_header(
        "Demo 03: Biobank Screen-TRex — MC SNR Sweep (In-Memory)");
    ...
    for (std::size_t snr_idx = 0; snr_idx < snr_values.size(); ++snr_idx) {
        ...
        for (std::size_t mc = 0; mc < num_MC; ++mc) {
            ...
            fdp_map[result.method_used](si)  += rates::compute_fdp(
                result.selected_indices, true_support);
            tpp_map[result.method_used](si)  += rates::compute_tpp(
                result.selected_indices, true_support);
            usage_map[result.method_used](si) += 1.0;

            // Estimated FDR values accumulated unconditionally for all methods
            est_fdr_map["Screen-TRex (ordinary)"](si)     +=
                                                        result.estimated_FDR_screen_ordinary;
            est_fdr_map["Screen-TRex (bootstrap-CI)"](si) +=
                                                        result.estimated_FDR_screen_bootstrap;
    ...
    stem << "d04_biobank_screen_trex_mc_snr_n" << n << "_p" << p
...
// ==============================================================================
// Demo 04 — MC SNR sweep — multiple phenotypes
// ==============================================================================

void demo_Biobank_MultiplePhenotypesMC(std::size_t num_MC)
{
    std::cout.setf(std::ios::unitbuf);
    cdiag::print_section_header(
        "Demo 04: Biobank Screen-TRex — MC SNR Sweep, Multiple Phenotypes (In-Memory)");
    ...
    // ── Build file stem and save results ───────────────────────────────────
    std::ostringstream stem;
    stem << "d04_biobank_screen_trex_mc_multi_n" << n << "_p" << p
