...
// ==============================================================================
/**
 * @file demo_trx_scr_03_screen_trex_correlated.cpp
 *
 ...
 *  Demo 06 — rho sweep: DA-AR1 on AR(1) data.
    ...
    for (const auto& method : methods) {
        ...
        for (std::size_t snr_idx = 0; snr_idx < snr_values.size(); ++snr_idx) {
            ...
            for (std::size_t mc = 0; mc < num_MC; ++mc) {
                ...
                auto data = dgp_ar1(n, p, p1, rho, snr, beta_val,
                ...
                total_fdp     += rates::compute_fdp(selected,
                                                    data.true_support);
                total_tpp     += rates::compute_tpp(selected,
                                                    data.true_support);
    ...
    for (const auto& method : methods) {
        ...
        for (std::size_t snr_idx = 0; snr_idx < snr_values.size(); ++snr_idx) {
            ...
            for (std::size_t mc = 0; mc < num_MC; ++mc) {
                ...
                auto data = dgp_equi(n, p, p1, rho, snr, beta_val,
                    static_cast<unsigned>(42 + snr_idx * 1000 + mc));
 ...
 *        on AR(1)-correlated data at fixed SNR.
 *
    ...
    for (const auto& method : methods) {
        ...
        for (std::size_t rho_idx = 0; rho_idx < rho_values.size(); ++rho_idx) {
            ...
            for (std::size_t mc = 0; mc < num_MC; ++mc) {
                ...
                print_mc_progress(rho, mc, num_MC, "rho");
            }
            print_mc_done(rho, num_MC, "rho");
 ...
 *        on equicorrelated data at fixed SNR.
 *
 * @param num_MC    Number of Monte Carlo runs per (method, rho) pair.
 * @param snr       Fixed signal-to-noise ratio.
 * @param beta_val  Coefficient magnitude for active variables.
    ...
    double      beta_val = 1.0)
{
    ...
    for (const auto& method : methods) {
        ...
        for (std::size_t rho_idx = 0; rho_idx < rho_values.size(); ++rho_idx) {
            ...
            for (std::size_t mc = 0; mc < num_MC; ++mc) {
                auto data = dgp_block_equi(n, p, p1, rho, n_blocks, snr,
                                           beta_val,
                                           static_cast<unsigned>(
                                               42 + rho_idx * 1000 + mc));
