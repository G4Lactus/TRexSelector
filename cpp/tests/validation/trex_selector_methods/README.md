...
depends on co-located in the same folder.
...
| `validation_trex_spca_04_rdump_pipeline` | `trex_spca/` | Step-by-step C++/R diff; needs an external R dump via `--dir` (~12 MB, not committed) |
...
| `validation_trex_spca_07_matrix_dump` | `trex_spca/` | Dumps GVS Phi/FDP calibration matrices; needs an external R dump via `--dir` |
